...
func SnapshotCmd() cli.Command {
	return cli.Command{
		...
		Subcommands: []cli.Command{
			...
			SnapshotHashInProgressTasksCmd(),
		},
...
func SnapshotHashInProgressTasksCmd() cli.Command {
	return cli.Command{
		Name: "hash-in-progress-tasks",
		Action: func(c *cli.Context) {
			if err := HashSnapshotInProgressTasks(c); err != nil {
				logrus.Fatalf("Error running snapshot hash status command: %v", err)
			}
		},
	}
...
func HashSnapshotInProgressTasks(c *cli.Context) error {
	if c.NArg() == 0 {
		return errors.New("snapshot name is required")
	}
