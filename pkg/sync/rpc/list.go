...
type SnapshotHashList struct {
	sync.RWMutex
	jobs []*SnapshotHashInfo
}

type SnapshotHashInfo struct {
	snapshotName string
	info         *replica.SnapshotHashJob
}
...
func (s *SnapshotHashList) Add(snapshotName string, info *replica.SnapshotHashJob) error {
	if snapshotName == "" {
	...
	for index, task := range s.jobs {
		if task.snapshotName == snapshotName {
			...
			if task.info.State == replica.ProgressStateComplete ||
				...
				updatedList, err := s.remove(s.jobs, index)
				if err != nil {
					return err
				}
				s.jobs = updatedList
	...
	s.jobs = append(s.jobs, &SnapshotHashInfo{
...
func (s *SnapshotHashList) Refresh() error {
	...
	purgedStates := map[replica.ProgressState]struct{}{
		replica.ProgressStateComplete: {},
		replica.ProgressStateError:    {},
	}

	return s.purgePartialRetained(purgedStates, MaxSnapshotHashJobSize)
}

func (s *SnapshotHashList) purgePartialRetained(purgedStates map[replica.ProgressState]struct{}, limit int) error {
	for state := range purgedStates {
		var index, completed int

		for index = len(s.jobs) - 1; index >= 0; index-- {
			if s.jobs[index].info.State == state {
				if completed == limit {
					break
				}
				completed++
		...
		if completed == limit {
			// Remove all the older completed or error jobs in the range snapshotHashList[0:index]
			for ; index >= 0; index-- {
				if s.jobs[index].info.State == state {
					updatedList, err := s.remove(s.jobs, index)
					if err != nil {
