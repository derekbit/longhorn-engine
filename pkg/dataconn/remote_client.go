...
// NewRemoteClient replica client using network protocol
func NewRemoteClient(conn net.Conn, engineToReplicaTimeout time.Duration) (DataConnClient, error) {
	c := &RemoteClient{
		wire:      NewWire(conn),
...
func (c *RemoteClient) targetID() string {
	return c.peerAddr
}

// WriteAt replica client
func (c *RemoteClient) WriteAt(buf []byte, offset int64) (int, error) {
	return c.operation(TypeWrite, buf, uint32(len(buf)), offset)
}

// UnmapAt replica client
func (c *RemoteClient) UnmapAt(length uint32, offset int64) (int, error) {
	return c.operation(TypeUnmap, nil, length, offset)
}

// SetError replica client transport error
func (c *RemoteClient) SetError(err error) {
	c.responses <- &Message{
		transportErr: err,
	}
}
...
func (c *RemoteClient) ReadAt(buf []byte, offset int64) (int, error) {
...
func (c *RemoteClient) operation(op uint32, buf []byte, length uint32, offset int64) (int, error) {
	msg := Message{
...
func (c *RemoteClient) nextSeq() uint32 {
	c.seq++
	return c.seq
}

func (c *RemoteClient) replyError(req *Message, err error) {
...
func (c *RemoteClient) handleRequest(req *Message) {
	switch req.Type {
	case TypeRead:
...
func (c *RemoteClient) handleResponse(resp *Message) {
