...
func (d *Factory) Create(volumeName, address string, engineToReplicaTimeout time.Duration) (types.Backend, error) {
	parts := strings.SplitN(address, "://", 2)

	if len(parts) == 2 {
		if factory, ok := d.factories[parts[0]]; ok {
			return factory.Create(volumeName, parts[1], engineToReplicaTimeout)
		}
	}
