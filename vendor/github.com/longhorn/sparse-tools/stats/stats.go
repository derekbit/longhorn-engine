...
func (sample dataPoint) String() string {
	...
	if sample.duration != time.Duration(0) {
		if sample.status {
			return fmt.Sprintf("%s: REQ %v ->%s %v[%3dkB@%v] %8dus", sample.timestamp.Format(time.StampMicro), sample.seq, target, sample.op, sample.size/1024, sample.offset, sample.duration.Nanoseconds()/1000)
		}
		return fmt.Sprintf("%s: REQ %v ->%s %v[%3dkB@%v] %8dus failed", sample.timestamp.Format(time.StampMicro), sample.seq, target, sample.op, sample.size/1024, sample.offset, sample.duration.Nanoseconds()/1000)
	}
	return fmt.Sprintf("%s: REQ %v ->%s %v[%3dkB@%v] pending", sample.timestamp.Format(time.StampMicro), sample.seq, target, sample.op, sample.size/1024, sample.offset)
}
